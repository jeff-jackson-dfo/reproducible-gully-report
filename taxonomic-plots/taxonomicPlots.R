...
# Load the Gully biological taxonomic data.
springAvg <- readxl::read_excel("data/copepod-abundances.xlsx", sheet = "Spring Avg")
springSD <- readxl::read_excel("data/copepod-abundances.xlsx", sheet = "Spring StDev")
fallAvg <- readxl::read_excel("data/copepod-abundances.xlsx", sheet = "Fall Avg")
fallSD <- readxl::read_excel("data/copepod-abundances.xlsx", sheet = "Fall StDev")
...
g <- ggplot(data = sTaxo, aes(x = avg, y = reorder(taxonomy, desc(taxonomy)), fill = station)) +
...
print(g)
ggsave("figure31.png", g, height = 8.5, width = 7)

# Produce a bar plot for the fall taxonomic data grouped by station
g <- ggplot(data = fTaxo, aes(x = avg, y = reorder(taxonomy, desc(taxonomy)), fill = station)) +
  geom_bar(stat="identity", width = 0.7, position=position_dodge()) +
  geom_errorbar(aes(xmin = pmax(avg - sd, 0), xmax = avg + sd), position=position_dodge(.7), width = 0) +
...
ggsave("figure32.png", g, height = 8.5, width = 7)
